...
@task
def reset_and_fill_db(context):
    """Reset database and then fill it with sample data."""
    resetdb(context)
    common.success("Filling database with initial data")
    manage(context, "fill_sample_data")


@task
def add_sample_data(context):
    """Add sample data to database without reset."""
    common.success("Adding sample data to database")
    manage(context, "fill_sample_data")
...
@task
def run(context):
    """Run development web-server over https."""
    ...
    manage(
        context,
        "runserver_plus localhost:8000 --reloader-type stat",
    )
